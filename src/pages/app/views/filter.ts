...
import {
  type FilterBlock, type Condition, type RGB, type Style,
  CONDITION_FIELDS, NUMERIC_OPS, STRING_OPS, RARITIES, ICON_COLORS, ICON_SHAPES,
  loadBlocks, saveBlocks, serialize, serializeFilter, parseFilter, emptyBlock,
} from '../filter';
...
function serializeCurrent(): string {
  const imported = PREAMBLE.length > 0 ||
    BLOCKS.some((b) => b.comments?.length || b.headerComment || b.unknown?.length || b.cont);
  return imported ? serializeFilter({ preamble: PREAMBLE, blocks: BLOCKS }) : serialize(BLOCKS);
}
...
const FIELD_ZH: Record<string, string> = {
  Class: '類別', BaseType: '基底名', Rarity: '稀有度', ItemLevel: '物品等級', DropLevel: '掉落等級',
  AreaLevel: '區域等級', Quality: '品質', StackSize: '堆疊數', Sockets: '插槽', LinkedSockets: '連線數',
  MapTier: '地圖階級', GemLevel: '寶石等級', Corrupted: '已汙染', Identified: '已鑑定',
};
const RARITY_ZH: Record<string, string> = { Normal: '普通', Magic: '魔法', Rare: '稀有', Unique: '傳奇' };

// 常見過濾器 Class → 繁中（顯示層；未列出者原樣顯示英文，不影響產出）。
const CLASS_ZH: Record<string, string> = {
  Currency: '通貨', 'Divination Card': '命運卡', 'Stackable Currency': '可堆疊通貨',
  Maps: '地圖', 'Map Fragments': '地圖碎片', 'Misc Map Items': '雜項地圖物品',
  Jewels: '珠寶', 'Abyss Jewel': '深淵珠寶', 'Cluster Jewel': '星團珠寶',
  'Body Armours': '胸甲', Boots: '鞋子', Gloves: '手套', Helmets: '頭盔', Shields: '盾',
  Rings: '戒指', Amulets: '護身符', Belts: '腰帶', Quivers: '箭袋',
  Bows: '弓', Wands: '法杖', Daggers: '匕首', 'Rune Daggers': '符文匕首', Claws: '爪',
  'One Hand Swords': '單手劍', 'Two Hand Swords': '雙手劍', 'Thrusting One Hand Swords': '細劍',
  'One Hand Axes': '單手斧', 'Two Hand Axes': '雙手斧', 'One Hand Maces': '單手錘', 'Two Hand Maces': '雙手錘',
  Sceptres: '權杖', Staves: '長杖', Warstaves: '戰杖', 'Fishing Rods': '釣竿',
  'Life Flasks': '生命藥劑', 'Mana Flasks': '魔力藥劑', 'Hybrid Flasks': '複合藥劑', 'Utility Flasks': '功能藥劑',
  'Active Skill Gems': '主動技能寶石', 'Support Skill Gems': '輔助技能寶石',
  'Skill Gems': '技能寶石', Gems: '寶石',
  'Heist Gear': '劫盜裝備', 'Heist Tools': '劫盜工具', 'Heist Cloaks': '劫盜披風', 'Heist Brooches': '劫盜胸針',
  'Contract': '契約', 'Blueprint': '藍圖', Trinkets: '飾物', 'Pieces': '碎片',
};
...
function condHint(b: FilterBlock): string {
  ...
  const lk = find('LinkedSockets'); if (lk?.value) return `${lk.value} 連線`;
  const sk = find('Sockets'); if (sk?.value) return `插槽 ${sk.value}`;
  const mt = find('MapTier'); if (mt?.value) return `地圖階級 ${mt.op} ${mt.value}`.replace(/\s+/g, ' ').trim();
  const ss = find('StackSize'); if (ss?.value) return `堆疊 ${ss.op} ${ss.value}`.replace(/\s+/g, ' ').trim();
  const rr = find('Rarity'); if (rr?.value) return `稀有度 ${rr.op} ${RARITY_ZH[rr.value] ?? rr.value}`.replace(/\s+/g, ' ').trim();
  const il = find('ItemLevel'); if (il?.value) return `物品等級 ${il.op} ${il.value}`.replace(/\s+/g, ' ').trim();
  return '';
}
...
function rgbToHex(c: RGB): string {
  ...
  return '#' + c.slice(0, 3).map((n) => Math.max(0, Math.min(255, n)).toString(16).padStart(2, '0')).join('');
}
...
function condSummary(b: FilterBlock): string {
  const extra = b.unknown?.length ? `<span class="filt-adv">+${b.unknown.length} 進階</span>` : '';
  ...
  const parts = b.conditions.map((c) => {
    ...
    const v = toks.length > 4
      ? toks.slice(0, 4).map((t) => t.replace(/"/g, '')).join(' ') + ` …等 ${toks.length} 項`
      : (c.value || '').replace(/"/g, '');
...
interface Group { name: string; items: { b: FilterBlock; i: number }[]; }
...
function computeGroups(): Group[] {
  ...
  BLOCKS.forEach((b, i) => {
    ...
    if (marker) { cur = { name: marker, items: [] }; groups.push(cur); }
    if (!cur) { cur = { name: '（未分節）', items: [] }; groups.push(cur); }
    cur.items.push({ b, i });
  });
  return groups;
}
...
function matchBlock(b: FilterBlock, q: string): boolean {
  if (!q) return true;
  const hay = [
    b.name, b.headerComment ?? '', b.action, cardTitle(b),
...
function listHtml(): string {
  ...
  groups.forEach((g, gi) => {
    ...
    const cnt = q && matched.length !== g.items.length ? `${matched.length}/${g.items.length}` : `${g.items.length}`;
    out.push(`
  ...
  return out.length ? out.join('') : '<div class="filt-empty">查無符合的規則。</div>';
}
...
function blockCard(b: FilterBlock, i: number): string {
  ...
  const actZh = b.action === 'Show' ? '顯示' : b.action === 'Hide' ? '隱藏' : '精簡';
...
function condRow(c: Condition, idx: number): string {
  ...
  const fieldOpts = CONDITION_FIELDS.map(
    (f) => `<option value="${f.key}" ${f.key === c.field ? 'selected' : ''}>${FIELD_ZH[f.key] ?? f.key}</option>`,
  ).join('');
  const opOpts = ops.map((o) => `<option value="${o}" ${o === c.op ? 'selected' : ''}>${o || '包含'}</option>`).join('');
  // Rarity 用下拉，其餘用文字
  let valInput: string;
  if (c.field === 'Rarity') {
    valInput = `<select class="filt-in" data-cond="val" data-idx="${idx}">${RARITIES.map(
      (r) => `<option value="${r}" ${r === c.value ? 'selected' : ''}>${RARITY_ZH[r]}</option>`,
    ).join('')}</select>`;
  } else {
...
// ── 編輯器：樣式列（顏色） ─────────────────────────────────────────────────────
function colorRow(label: string, key: 'textColor' | 'borderColor' | 'bgColor', s: Style): string {
  const v = s[key];
...
function editor(b: FilterBlock): string {
  ...
  const iconColorOpts = ICON_COLORS.map((c) => `<option value="${c}" ${icon?.color === c ? 'selected' : ''}>${c}</option>`).join('');
  const iconShapeOpts = ICON_SHAPES.map((c) => `<option value="${c}" ${icon?.shape === c ? 'selected' : ''}>${c}</option>`).join('');
  const beamColorOpts = ICON_COLORS.map((c) => `<option value="${c}" ${beam?.color === c ? 'selected' : ''}>${c}</option>`).join('');
  return `
    <div class="filt-ed">
...
export const filter: View = {
  render() {
    ensureLoaded();
    const b = selected();
    const list = BLOCKS.length ? listHtml() : '<div class="filt-empty">尚無規則，按上方新增或讀取。</div>';
      ...
      <div class="filt-wrap">
        ...
        <div class="filt-rail">
          <div class="panel filt-ed-panel">
            ...
            ${b ? editor(b) : '<div class="filt-empty pad">選擇左側規則以編輯，或新增一條。</div>'}
          </div>

          <div class="panel filt-out-panel">
            <div class="panel-bar">
              <span class="name">輸 出 · .filter</span>
              <div class="filt-toolbar">
                <button class="btn" id="copy-out">複製</button>
                <button class="btn btn-dark" id="dl-out">下載</button>
              </div>
            </div>
            <textarea class="filt-out" id="filt-out" readonly spellcheck="false">${serializeCurrent().replace(/</g, '&lt;')}</textarea>
          </div>
        </div>
      </div>`;
  },
  ...
  mount(root) {
    ...
    root.querySelector('#add-show')?.addEventListener('click', () => {
      const nb = emptyBlock('Show'); BLOCKS.push(nb); selectedId = nb.id; persist(); rerender();
    });
    root.querySelector('#add-hide')?.addEventListener('click', () => {
      const nb = emptyBlock('Hide'); BLOCKS.push(nb); selectedId = nb.id; persist(); rerender();
    });
    ...
    // 搜尋：只局部刷新清單，保留輸入焦點。
    root.querySelector<HTMLInputElement>('#filt-search')?.addEventListener('input', (e) => {
    ...
    root.querySelector('#copy-out')?.addEventListener('click', async () => {
      try { await navigator.clipboard.writeText(serializeCurrent()); } catch { /* ignore */ }
    });
    ...
    root.querySelector<HTMLInputElement>('#ed-name')?.addEventListener('change', (e) => {
      b.name = (e.target as HTMLInputElement).value; persist(); rerender();
    });
    // Show/Hide
    root.querySelectorAll<HTMLElement>('[data-action]').forEach((el) =>
      el.addEventListener('click', () => { b.action = el.dataset['action'] as 'Show' | 'Hide'; persist(); rerender(); }),
    );

    // 條件：新增
    root.querySelector('#add-cond')?.addEventListener('click', () => {
      b.conditions.push({ field: 'BaseType', op: '', value: '' }); persist(); rerender();
    });
    // 條件：欄位/運算子/值/刪除
    root.querySelectorAll<HTMLElement>('[data-cond]').forEach((el) => {
      ...
      if (kind === 'del') {
        el.addEventListener('click', () => { b.conditions.splice(idx, 1); persist(); rerender(); });
      } else if (kind === 'field') {
        el.addEventListener('change', (e) => {
          const c = b.conditions[idx]!; c.field = (e.target as HTMLSelectElement).value;
          const def = CONDITION_FIELDS.find((f) => f.key === c.field);
          // 切換數值/字串時，運算子與值重設為合理預設
          if (def?.numeric) { if (!NUMERIC_OPS.includes(c.op)) c.op = '>='; }
          else { c.op = ''; }
          if (c.field === 'Rarity' && !RARITIES.includes(c.value)) c.value = 'Rare';
          persist(); rerender();
        });
      } else if (kind === 'op') {
        el.addEventListener('change', (e) => { b.conditions[idx]!.op = (e.target as HTMLSelectElement).value; persist(); rerender(); });
      } else if (kind === 'val') {
        el.addEventListener('change', (e) => { b.conditions[idx]!.value = (e.target as HTMLInputElement | HTMLSelectElement).value; persist(); refreshOut(root); });
      }
    });
    ...
    root.querySelectorAll<HTMLInputElement>('[data-style-on]').forEach((el) =>
      el.addEventListener('change', () => {
        ...
        if (el.checked) {
          ...
          else if (key === 'minimapIcon') b.style.minimapIcon = { size: 0, color: 'White', shape: 'Circle' };
          else if (key === 'beam') b.style.beam = { color: 'White', temp: false };
          else if (key === 'alertSound') b.style.alertSound = { id: 1, volume: 100 };
        } else {
          delete b.style[key];
        }
        persist(); rerender();
    ...
    root.querySelectorAll<HTMLInputElement>('[data-style-color]').forEach((el) =>
      el.addEventListener('input', () => {
        const key = el.dataset['styleColor'] as 'textColor' | 'borderColor' | 'bgColor';
        b.style[key] = hexToRgb(el.value); persist(); livePreview(root, b); refreshOut(root);
      }),
    );
    ...
    fs?.addEventListener('input', () => {
      ...
      const lbl = root.querySelector('#fs-val'); if (lbl) lbl.textContent = fs.value;
      persist(); livePreview(root, b); refreshOut(root);
    });
...
function livePreview(root: HTMLElement, b: FilterBlock): void {
  ...
  root.querySelectorAll<HTMLElement>(`.filt-prev.big, [data-prev="${b.id}"]`).forEach((el) => el.setAttribute('style', css));
}
/** 只刷新輸出文字。 */
...
function refreshList(root: HTMLElement): void {
  ...
  if (listEl) { listEl.innerHTML = BLOCKS.length ? listHtml() : '<div class="filt-empty">尚無規則，按上方新增或讀取。</div>'; }
  bindListEvents(root);
}
...
function bindListEvents(root: HTMLElement): void {
  ...
  root.querySelectorAll<HTMLElement>('[data-pick]').forEach((el) =>
    el.addEventListener('click', (e) => {
      ...
      selectedId = el.dataset['pick']!; rerender();
    }),
  );
  // 卡片操作
...
function mountImport(root: HTMLElement): void {
  ...
  root.querySelector('#imp-cancel')?.addEventListener('click', () => {
    importOpen = false; rerender();
  });
  root.querySelector('#imp-parse')?.addEventListener('click', () => {
    const ta = root.querySelector<HTMLTextAreaElement>('#imp-text');
    const merge = root.querySelector<HTMLInputElement>('#imp-merge')?.checked ?? false;
    const text = ta?.value ?? '';
    if (!text.trim()) { importMsg = '⚠ 請先貼上內容或開啟檔案。'; rerender(); return; }
    doImport(text, merge);
  });
